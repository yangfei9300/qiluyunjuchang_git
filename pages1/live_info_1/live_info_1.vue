<template>
	<view>
		...
		<view class="w-750 asasas" v-if="!uanping"
			style="height:100vh;z-index: 0;position: fixed;top: 0rpx;background-color: #FF8E2D;" mode="aspectFill"
			:style="{
			 'background-image': 'url('+liveInfoInfo.live_banner+')'
		 }"></view>

		<view class="topallview " style="z-index:500;min-height: 150rpx;" v-if="!uanping" :style="{
			...
			<view class="capsulevioew roww center_center" :style="{
				...
				<view hover-class="none" @click.stop="toLive" style="padding-right: 30rpx;">
					<!-- @click.stop="backpage"  -->
					<image src="../../static/zuojiantou.png" :style="{
				...
				<view class="roww center_center infoview" style="padding:5rpx 0rpx;" :style="{
					...
					<image :src="liveInfoInfo.live_banner" mode="aspectFill" class="dramaIcon"></image>
					<view class="colonn allline">
						<view class="fs-20 color6_r txtShowLength">{{liveInfoInfo.live_name?liveInfoInfo.live_name:''}}
						</view>
						<view class="popularity m-top-5">
							{{liveInfoInfo.live_max_member?liveInfoInfo.live_max_member:0}}人气值</view>
					</view>
					<view class="w-20"></view>
				</view>
		...
		<view class="colonn" style="position: fixed;" :style="{
			...
			<view class="roww p-all-15" v-if="!uanping">
				<!--  #ifdef  MP-WEIXIN -->
				<!-- <view class="roww zhibotixingview rowsa center_center" @click.stop="kaiboTixing">
					<image src="/static/live/zhibxq_03.png" class="naozhogicon"></image>
					<view>直播提醒</view>
				</view> -->
				<!--  #endif -->
				...
				<view class="allline"></view>
				<view class="gengduojieshao roww center_center" @click.stop="liveinfoalertlick">
					<view>更多介绍</view>
					<image src="/static/live/xiajiantou2.png" class="w-30 h-30 m-left-10"></image>
				</view>
			</view>
			<!-- 视频 -->
			...
			<!-- v-show="!loading" -->
			<!-- <live-player  
			 src="rtmp://r2bj.vzan.com/v/1017132112_741089763015668852?zbid=1017132112&tpid=1707316049"
			 mode="RTC" 
			...
			@statechange="statechange"
			 @error="error"
			  ...
			  objectFit="contain"
			 @netstatus="netstatus"
			 style="width:750rpx;height:430rpx;" ></live-player>
			     -->
			<!-- rtmp://r2bj.vzan.com/v/1017132112_741089763015668852?zbid=1017132112&tpid=1707316049 -->
			<!--  src="rtmp://play-pc.ourwill.cn/live/tw-1909637-bcmrz0f0_hd?txSecret=2a5f8e3b824427a387925a1b7ee840f0&txTime=62C2482F"  -->
			<!-- live-player -->
			<!-- rtmp://play-pc.ourwill.cn/live/tw-1928170-xvxg4n55_hd?txSecret=451c0f57a35eb0688a04fd3db8778456&txTime=62CA95A3 -->

			<!--  :src="liveInfoInfo.live_apply_video" -->
			<live-player v-if="!liveInfoInfo.mu38" id="liveplayerid" class="pore" :src="liveInfoInfo.live_apply_video"
				:class="{
				 'henghea':uanping,
				 'uanpinga':!uanping,
			 }" object-fit="contain" autoplay min-cache="10" max-cache="30" @statechange="statechange" @netstatus="netstatus"
				@error="error" picture-in-picture-mode="push" :orientation="!uanping?'vertical':'horizontal'">
				<!-- v-if="!uanping" -->
				<image :src="liveInfoInfo.live_banner" style="width:750rpx;height:430rpx;" v-show="loading&&!uanping"
			...
			<video v-if="liveInfoInfo.mu38" id="liveplayerid" :poster="liveInfoInfo.live_banner"
				:orientation="!uanping?'vertical':'horizontal'" class="pore" :src="liveInfoInfo.live_apply_video"
				:class="{
				 'henghea':uanping,
				 'uanpinga':!uanping,
			 }" object-fit="contain" :show-fullscreen-btn="true" autoplay show-fullscreen-btn="true"
				picture-in-picture-mode="push">
				<!-- v-if="!uanping" -->
				<!-- <image :src="liveInfoInfo.live_banner"
		...
		<!-- <view class="roww poab m-left-20" 
>
	<view class="animate rucahngeiuview" >入场特效</view>
</view> -->



		<!-- 聊天内容 -->
		<view style="width:750rpx;padding-left:20rpx;z-index: 1001;" v-if="!uanping" :style="{
			height:chatviewhieght+'px',
			'margin-top':chatview_top+'px'
		}">
			<scroll-view :scroll-into-view="scrooid" scroll-y :style="{
				height:chatviewhieght+'px',
			}">
				<view class="colonn">

					<!-- 普通用户聊天 -->
					<view class=" roww " style="max-width: 500rpx;" v-if="tonggaoTxt.content||tonggaoTxt.image">
						<view class="colonn chatitem">
							...
							<view class="  fs-30 liaotianitem1"
								...
								{{tonggaoTxt.from_client_name}}：
								<text
									style="line-height:40rpx;color: red;font-size:30rpx;">{{tonggaoTxt.content}}</text>
							</view>
							<view class="   liaotianitem1"
								style="color: #FF8E2D;padding-left:10rpx;padding-right:10rpx;font-size:30rpx;" :class="{
								'tupians':tonggaoTxt.content==''
							}">
								<block v-if="tonggaoTxt.content==''">
									<text class="yincang1">{{tonggaoTxt.from_client_name}}：</text>
								</block>
								<image @click.stop="showimggongaoimg(tonggaoTxt.image)" mode="aspectFill"
									:src="tonggaoTxt.image" v-if="tonggaoTxt.image"
									class="w-200 m-top-10 m-left-00 m-bottom-10 h-200"></image>
							</view>
						</view>
							...
							<view class=" roww " style="max-width: 500rpx;">
								<view class=" fs-30 chatitem liaotianitem1"
									...
									<text style="line-height:40rpx;color: #ffffff;font-size:30rpx;"
										v-if="item.userinfo.type==0">{{item.content}}</text>
									<text style="line-height:40rpx;color: red;font-size:30rpx;"
										v-if="item.userinfo.type==1">{{item.content}}</text>
									<text style="color: red;padding-left:10rpx;" v-if="chatMsgUserInfo.type==1"
										@click.stop="sendDelMessage(item)">删除</text>
								</view>
								<view class="allline"></view>
							</view>
						</block>
						...
						<block v-if="item.type=='say'&&item.image">
							<view class="roww">
								<view class="fs-30 center_center roww chatitem">
									<text
										style="color: #FF8E2D;padding-left:10rpx;">{{item.userinfo.client_name}}：</text>
									<image class="chatimg" mode="aspectFill" :src="item.image">
									</image>
									<text style="color: red;padding-left:10rpx;" v-if="chatMsgUserInfo.type==1"
										@click.stop="sendDelMessage(item)">删除</text>
								</view>
		...
		<view class="roww rowsb p-left-25 p-right-25" style="position: fixed;bottom:72rpx;width:750rpx;"
			...
			<block v-if="liveInfoInfo.is_message==1">
				...
				<block v-if="chatMsgUserInfo.client_name=='游客'">
					<view class="shurukuangview pore roww center_center" @click.stop="getuserinfoBtn">
						<view class="w-20"></view>
						<view class="shuodianshenm">请先授权..</view>
						<view class="allline"></view>
						<image src="/static/live/biaoqing.png" class="w-38 h-38"></image>
						<!-- <button open-type="getUserInfo" @getuserinfo="getuserinfoBtn"
							 style="width: 100%;height: 100%;position: absolute;left: 0rpx;top: 0rpx;opacity: 0;"
							 ></button> -->
					</view>
				</block>
				<block v-else>
					<view class="shurukuangview roww center_center" @click.stop="menuTypeClick">
						<view class="w-20"></view>
						<view class="shuodianshenm">说点什么..</view>
						<view class="allline"></view>
						<image src="/static/live/biaoqing.png" @click.stop="selEmiloAlert=true" class="w-38 h-38">
						</image>
					</view>
				</block>
			...
			<view class="liwuclick roww center_center">
				<image src="/static/live/zhibxq_10.png" v-if="zanLiveIs" class="w-39 h-39"></image>
				<image src="/static/live/wuxin.png" @click.stop="toDianzan(1)" v-else class="w-39 h-39"></image>
			</view>
		</view>

		<view class="colonn livebigvi animated " v-if="liveinfoalert">
			<view class="roww rowsb w-710 p-bottom-15">
				...
				<image class="w-50 h-50" @click.stop="liveinfoalertlick"
					src="../../static/371477a4437cdd68956cc39b35bdc05.png"></image>
			</view>
			<scroll-view scroll-y style="height:360rpx;">
				<view class="liveinfo" style="color: white;font-size: 30rpx;">
					<rich-text :nodes="liveInfoInfo.live_description"></rich-text>
				</view>
			</scroll-view>
		</view>

		<view class="huuibeijing" v-if="selEmiloAlert">
			<view style="width: 750rpx;position: fixed;bottom: 0rpx;" class="colonn">
				<view class="roww  center_center">
					<image src="../../static/guanbi.png" @click.stop="selEmiloAlertClick" class="w-70 h-70 m-bottom-30">
					</image>
				</view>
				<view class="background1 colonn" style="height: 400rpx;">

					<!-- <view class="roww " style="padding:15rpx 33rpx;">
				<image lazy-load 
				:src="selEmilo" 
				class="w-55 h-55"></image>
				<view class="allline"></view>
				<view class="fs-25" @click="sendMessage('img',selEmilo)">发送</view>
			</view> -->
					<!-- style="height: 720rpx;" -->
					...
					<swiper class="w-750 h-400">
						<swiper-item class="w-750 h-400">
							<view class="huanhang">
								<view class="colonn " style="padding:15rpx 10rpx;"
									v-for="(item,index) in biaoqingList.png"
									@click="sendMessage('img',item.expression_url)" v-if="index<50">
									<image lazy-load mode="aspectFill" :src="item.expression_url" class="w-55 h-55">
									</image>
								</view>
							</view>
						</swiper-item>
						<swiper-item class="w-750 h-400">
							<view class="huanhang">
								<view class="colonn " style="padding:15rpx 10rpx;"
									v-for="(item,index) in biaoqingList.png"
									@click="sendMessage('img',item.expression_url)" v-if="index>=50&&index<100">
									<image lazy-load :src="item.expression_url" class="w-55 h-55"></image>
								</view>
							</view>
						</swiper-item>
						<swiper-item class="w-750 h-400">
							<view class="huanhang">
								<view class="colonn " style="padding:15rpx 10rpx;"
									v-for="(item,index) in biaoqingList.png"
									@click="sendMessage('img',item.expression_url)" v-if="index>100">
									<image lazy-load :src="item.expression_url" class="w-55 h-55"></image>
								</view>
							</view>
						</swiper-item>
					...
					<!-- <scroll-view
			scroll-y class=" colonn"  
			style="height: 500rpx;"
			>
				<view class="huanhang "
				style="padding-left:18rpx;"
				>
					<view class="colonn " style="padding:15rpx 10rpx;"
					v-for="(item,index) in biaoqingList.png"
					 @click="sendMessage('img',item.expression_url)"
					>
						<image lazy-load :src="item.expression_url" class="w-55 h-55"></image>
					</view>
				</view>
		...
		<view class="roww background1 width center_center" style="padding:20rpx; 20rpx;position: fixed;" :style="{
			'bottom':keyheight+'px'
		}" v-if="menuType">
			<view style="width:10rpx;"></view>
			<view class="allline" style="background-color: #f5f5f5;border-radius: 100rpx;padding:10rpx 20rpx;">
				<view style="width:30rpx;"></view>
				<input placeholder="请输入内容" class="allline fs-30" @blur="focusInputblur" :focus="commeinptFocus"
					v-model="inputInfo" cursor-spacing="10" :adjust-position="false" placeholder-class=" fs-30" />
				<view style="width:30rpx;"></view>
			</view>
			<view style="width:25rpx;"></view>
			<view class="sendbtn" @click="sendMessage('say')">发送</view>
		</view>

		<!-- <view class="colonn shopmenu">
			<image class="shop_goodsicon" @click.stop="goodListShowClick"></image>
			<image class="shop_goodsicon"  @click.stop="toMyChat"></image>
		</view>   -->

		<!-- <goodList @clickAlert="clickAlert"
		  @showGoodInfo="showGoodInfo"
		  v-if="goodListShow"></goodList>
		 <goodInfo  @clickAlert="clickAlert"  v-if="goodInfoShow"></goodInfo>  -->
	</view>
</template>
<script>
	export default {
		data() {
			return {
				...
				// chatUrl:"ws://document.domain:8881",//聊天链接
				// 
		...
		onLoad(options) {
			...
			if (userInfo) {
				this.chatMsgUserInfo.client_name = userInfo.member_nickname ? userInfo.member_nickname : '游客';
				this.chatMsgUserInfo.avatar = userInfo.member_avatar;
				this.chatMsgUserInfo.type = userInfo.admin_state;
			} else {
		...
		methods: {
			...
			// 点击显示还是隐藏
			goodListShowClick() {
				this.goodListShow = !this.goodListShow;
			},
			goodInfoShowClick() {
				this.goodInfoShow = !this.goodInfoShow;
			},
			...
			statechange(res) {
				console.error("statechangestatechange", res.detail.code == 2004, res)
			...
			getEmild() {
				...
				this.$axios
					.axios('POST', this.$paths.expressionindex, data)
					.then(res => {
						if (res.code == 10000) {
							this.biaoqingList = res.result;
						} else {
							this.$tools.showToast(res.message);
						}
					})
			...
			initWebSocket() {
				...
				uni.onSocketClose(function(res) {
					console.log("聊天链接onSocketClose，",res);  
				...
				uni.onSocketError(function(res) {
					console.log("聊天链接onSocketError，",res);  
					uni.showToast({
			...
			initWebSocket_dy() {
				var _this = this
				this.taskWebsocket.onClose(function(res) {
					_this.ifConnect = false,
						uni.hideLoading()
					if (res.reason) {
				...
				this.taskWebsocket.onError(function(res) {
					uni.showToast({
						title: '聊天系统接入失败：' + res.errMsg,
						icon: 'none'
					});
				...
				this.taskWebsocket.onOpen(function(res) {
					uni.hideLoading()
					_this.ifConnect = true,
				...
				this.taskWebsocket.onMessage((res) => {
					uni.hideLoading()
					var message = JSON.parse(res.data)
			...
			getLiveInfo() {
				...
				this.$axios
					.axios('POST', this.$paths.live_detailIndex, data)
					.then(res => {
						if (res.code == 10000) {
							this.liveInfoInfo = res.result

							if (this.liveInfoInfo.live_apply_video && this.liveInfoInfo.live_apply_video.indexOf(
									"m3u8") >= 0) {
								this.liveInfoInfo.mu38 = true;
								this.videoContext = uni.createVideoContext('liveplayerid', this);
							}
			...
			// 发送通告内容
			sendTonggao() {
				var liveInfoInfo = this.liveInfoInfo;
				var tonggaoTxt = {
					content: liveInfoInfo.live_notice,
					from_client_name: "系统公告",
					type: 'system',
					image: liveInfoInfo.live_notice_img
				};
				this.tonggaoTxt = tonggaoTxt;
			},
			// ---------------------------对接接口结束-----------------------
			...
			getWindowInfo() {
				try {
					...
					// 最后30  是距离视频组件的距离
					this.chatview_top = menuButton.height + menuButton.top + 8 + uni.upx2px(450) + 30 + uni.upx2px(70);
				} catch (e) {}
			},
	...
	var heartCheck = {
		...
		start: function() {
			...
			this.timeoutObj = setInterval(res=> {
